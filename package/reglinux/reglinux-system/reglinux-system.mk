...
# system-update, system-config might need fix to deal with version string
...
define REGLINUX_SYSTEM_INSTALL_TARGET_CMDS
	...
	# datainit
        mkdir -p $(TARGET_DIR)/usr/share/reglinux
        rsync -arv $(BR2_EXTERNAL_REGLINUX_PATH)/package/reglinux/reglinux-system/datainit/ $(TARGET_DIR)/usr/share/reglinux/datainit/
	mkdir -p $(TARGET_DIR)/usr/share/reglinux/datainit/system
	cp $(BR2_EXTERNAL_REGLINUX_PATH)/package/reglinux/reglinux-system/system.conf \
	    $(TARGET_DIR)/usr/share/reglinux/datainit/system

	# system-boot.conf
	$(INSTALL) -D -m 0644 \
	    $(BR2_EXTERNAL_REGLINUX_PATH)/package/reglinux/reglinux-system/system-boot.conf \
		$(BINARIES_DIR)/system-boot.conf

	# sysconfigs (default system.conf for boards)
	mkdir -p $(TARGET_DIR)/usr/share/reglinux/sysconfigs
    if test -d \
	    $(BR2_EXTERNAL_REGLINUX_PATH)/package/reglinux/reglinux-system/sysconfigs/${REGLINUX_SYSTEM_ARCH}; \
		then cp -pr \
		$(BR2_EXTERNAL_REGLINUX_PATH)/package/reglinux/reglinux-system/sysconfigs/${REGLINUX_SYSTEM_ARCH}/* \
	...
	cp $(BR2_EXTERNAL_REGLINUX_PATH)/package/reglinux/reglinux-system/xdg.sh \
	    $(TARGET_DIR)/etc/profile.d/xdg.sh
	cp $(BR2_EXTERNAL_REGLINUX_PATH)/package/reglinux/reglinux-system/dbus.sh \
